  ...
  <div id="app">
    ...
    <div class="podcasts-wrapper">
      ...
      >
        ...
        <div class="action-buttons">
          <div class="button-container">
            <router-link
              ...
              :to="{
                name: 'UserReviews',
                params: { podcastUuid: podcast.uuid },
              }"
            ...
            <button
              v-if="podcast.userReview"
              @click="editReview(podcast.uuid, podcast.userReview)"
            >
              Edit Review
            </button>
            <button
              v-if="podcast.userReview"
              @click="deleteReview(podcast.uuid)"
            >
              Delete Review
            </button>
          </div>
        ...
        <button
          class="remove-podcast-button"
          @click="deletePodcast(podcast.uuid)"
        >
          X<span class="tooltip-text">Remove Podcast</span>
        </button>
      </div>
  ...
  methods: {
    async fetchPodcasts() {
      try {
        ...
        const response = await axios.post(
          "http://localhost:4000/user/podcasts",
            ...
            this.podcasts.map(async (podcast) => {
              try {
                const reviewResponse = await axios.post(
                  "http://localhost:4000/review/user",
                  {
                    userEmail: this.userEmail,
                ...
                if (
                  reviewResponse.data &&
                  "rating" in reviewResponse.data &&
                  "description" in reviewResponse.data
                ) {
                  podcast.userReview = reviewResponse.data;
                } else {
                  console.warn(
                    `No review data for podcast UUID: ${podcast.uuid}`
                  );
                ...
                if (
                  ...
                  console.warn(
                    "Review not found for podcast UUID:",
                    podcast.uuid
                  );
                  ...
                  console.error(
                    "Error fetching review for podcast UUID:",
                    podcast.uuid,
                    reviewError
                  );
                  podcast.userReview = {
    ...
    async deleteReview(podcastUuid) {
      try {
        ...
        await axios.delete("http://localhost:4000/review/delete", {
